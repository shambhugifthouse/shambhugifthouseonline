...
function RechargeTerminalApp() {
  ...
  const mobileOperators = [
    ...
    { id: 'airtel', name: 'Airtel 5G', color: '#E40000', badge: 'AIRTEL' },
  ...
  const dthOperators = [
    ...
    { id: 'airteldth', name: 'Airtel Digital TV', color: '#E40000', badge: 'AIRTEL TV' },
  ...
  const handleRechargeSubmit = () => {
    if (!number || number.length < 10) {
      setToast({ message: 'Please enter a valid 10-digit number', type: 'warning' });
      setTimeout(() => setToast(null), 3000);
      return;
    }
    if (!amount || parseFloat(amount) <= 0) {
      setToast({ message: 'Please select or enter a valid amount', type: 'warning' });
      setTimeout(() => setToast(null), 3000);
      return;
    }
  ...
  const processRecharge = () => {
    ...
    setToast({ message: `Recharge of ₹${amount} for ${number} completed successfully!`, type: 'success' });
      ...
      <div className="row g-4">
        ...
        <div className="col-lg-5">
          <div className="card border-0 shadow-sm rounded-4 p-4 bg-white">
            ...
            <div className="row g-2 mb-4">
              {currentOps.map((op) => (
                <div key={op.id} className="col-6">
                  <div
                    className={`p-3 rounded-3 border text-center transition-all cursor-pointer ${
                      operator === op.id ? 'border-warning bg-light shadow-xs' : 'border-light bg-white'
                    }`}
                    style={{ cursor: 'pointer' }}
                    onClick={() => setOperator(op.id)}
                  >
                    <span className="badge rounded-pill text-white px-3 py-1.5 mb-1.5 font-monospace" style={{ background: op.color }}>
                      {op.badge}
                    </span>
            ...
            <button
              className="btn btn-warning w-100 py-3 fw-extrabold rounded-3 shadow-sm font-brand-serif fs-5"
              onClick={handleRechargeSubmit}
              style={{ background: 'linear-gradient(135deg, #F5DA8A 0%, #E5B84B 100%)', border: 'none', color: '#09241B' }}
            >
              <i className="fa-solid fa-bolt me-2"></i> Instant Recharge Now
            </button>
          </div>
        </div>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4 bg-white h-100">
            ...
            <div className="overflow-auto pe-1" style={{ maxHeight: '360px' }}>
              {filteredPlans.map((plan) => (
                <div
                  key={plan.id}
                  className={`card border rounded-3 p-3 mb-2.5 transition-all cursor-pointer ${
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content rounded-4 border-0 shadow-lg p-3">
              <div className="modal-header border-0 pb-0">
                <h5 className="modal-title fw-bold font-heading">Confirm Recharge Transaction</h5>
              ...
              <div className="modal-body p-4 text-center">
                <div className="p-3 rounded-4 mb-3" style={{ background: '#09241B', color: '#E5B84B' }}>
                  ...
                  <small className="text-white-50">{operator.toUpperCase()} Operator</small>
                </div>
                <div className="fs-2 fw-extrabold text-success mb-3">₹{amount}</div>
                <p className="text-muted small">Please confirm payment collection from customer before processing recharge.</p>
                <div className="d-flex gap-2">
                  <button className="btn btn-light w-50 py-2.5 fw-bold" onClick={() => setIsConfirming(false)}>Cancel</button>
                  <button className="btn btn-success w-50 py-2.5 fw-bold" onClick={processRecharge}>Confirm & Pay</button>
                </div>
              </div>
            </div>
          </div>
